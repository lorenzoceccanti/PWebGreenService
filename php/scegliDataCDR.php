...
<body onload="return mouseisLeaving();">
    ...
    <form action="./scegliDataCDR.php" method="post">
...
<?php
  function stampaOrari(){
    ...
    $result = returnDBPrenotazioniCDR($calendar);
    ...
    else
    {
        ...
        {
            if($row['Minuti'] == 15)
                $nostampa[] = ($row['Ora'] + 0.25);
            else if($row['Minuti'] == 30)
                $nostampa[] = ($row['Ora'] + 0.5);
            else if($row['Minuti'] == 45)
                $nostampa[] = ($row['Ora'] + 0.75);
            else
                $nostampa[] = $row['Ora'];
        }
...
function stampaAppuntamentiLiberi($nostampa, $ngiri, $calendar){
    ...
    echo "<h3 id=\"h3bluemin\"> Si ricorda che è necessario confermare la prenotazione almeno 1h prima della fascia oraria a cui appartiene l'appuntamento </h3>";
    echo "<p id=\"pbluemin\"> Esempio: per confermare la prenotazione nella fascia oraria 13.00-13.59, va data conferma entro le ore 11.59 </p>";
    ...
    if(count($nostampa) < 15 and $nomeday !== "Sunday" and calcolaFestivitaItaliane($giornoStr) == 0) // Qui inserire l'eccezione sui festivi
    {
        ...
        $orarioInt = 5;
        for($time = $tempoIniziale ; $time <= 1140; $time = $time + 15)
        {
            $ore = $time / 60;
            for($i=0; $i < $ngiri; $i++)
            {
            ...
            if(($time % 60) == 0)
            {
                ...
                if( i == true){
                    location.href= './nuovaprenotazioneCDRConfirmed.php?orario=$ore:00&data=$calendarString'; 
                }
                \">".($ore).".00</li>";
            }
            else if(($time % 60) == 15)
            {
                ...
                var i = confirm('Si conferma la scelta effettuata?');
                if( i == true){
                    location.href= './nuovaprenotazioneCDRConfirmed.php?orario=$oremod:15&data=$calendarString'; 
                }
                \">".($oremod).".15</li>";
            }
            else if(($time % 60) == 30)
            {
                ...
                var i = confirm('Si conferma la scelta effettuata?');
                if( i == true){
                    location.href= './nuovaprenotazioneCDRConfirmed.php?orario=$oremod:30&data=$calendarString'; 
                }
                \">".($oremod).".30</li>";
            }
            else if(($time % 60) == 45)
            {
                ...
                var i = confirm('Si conferma la scelta effettuata?');
                if( i == true){
                    location.href= './nuovaprenotazioneCDRConfirmed.php?orario=$oremod:45&data=$calendarString'; 
                }
                \">".($oremod).".45</li>";
            }
        }
